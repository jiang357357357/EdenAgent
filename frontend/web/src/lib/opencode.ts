...
export function isApiFilePart(part: ApiPart): part is ApiFilePart {
  return part.type === 'file' && typeof (part as { mime?: unknown; url?: unknown }).mime === 'string' && typeof (part as { url?: unknown }).url === 'string';
}

export function isApiToolPart(part: ApiPart): part is ApiToolPart {
  return part.type === 'tool' && typeof (part as { tool?: unknown }).tool === 'string' && typeof (part as { state?: unknown }).state === 'object';
}
...
export function isApiPatchPart(part: ApiPart): part is ApiPatchPart {
  return part.type === 'patch' && typeof (part as { hash?: unknown }).hash === 'string' && Array.isArray((part as { files?: unknown }).files);
}
...
export function isApiRetryPart(part: ApiPart): part is ApiRetryPart {
  return part.type === 'retry' && typeof (part as { attempt?: unknown }).attempt === 'number' && typeof (part as { error?: unknown }).error === 'object';
}
...
export function isApiStepFinishPart(part: ApiPart): part is ApiStepFinishPart {
  return part.type === 'step-finish' && typeof (part as { reason?: unknown }).reason === 'string' && typeof (part as { cost?: unknown }).cost === 'number';
}
...
export function mapMessage(input: ApiMessage): MessageData {
  const text = input.parts
    .filter((part): part is ApiTextPart => isApiTextPart(part) && !(part as ApiTextPart & { synthetic?: boolean }).synthetic)
    .map((part) => part.text)
    .join('\n')
    .trim();
  const reasoning = input.parts
    .filter(isApiReasoningPart)
    .map((part) => part.text)
    .join('\n')
    .trim();
...
function createPromptParts(content: string, images: string[]) {
  return [
...
export async function sendPrompt(sessionID: string, content: string, images: string[]) {
  await request(`/session/${encodeURIComponent(sessionID)}/message`, {
    method: 'POST',
    body: JSON.stringify({
      parts: createPromptParts(content, images),
    }),
  });
}

export async function sendPromptAsync(sessionID: string, content: string, images: string[]) {
...
export async function subscribeEvents(handlers: SubscribeHandlers | ((event: ApiEvent) => void)) {
  const normalized: SubscribeHandlers =
