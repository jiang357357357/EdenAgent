...
import { readActivationPlan } from '../../packages/store/src/legacy/activation-plan.ts'
...
const activation = action === 'plan-activation'
const context = ['review-context', 'apply-context-summary'].includes(action)
const origin = args[status || context ? 1 : 2]
if (!['stage', 'resume', 'status', 'review-context', 'apply-context-summary', 'plan-activation'].includes(action) || !['mon', 'local'].includes(origin) ||
    (activation ? args.length !== 3 : context ? args.length !== 4 : status ? args.length !== 2 : args.length < 3 || args.length > 5) || args.some(value => !value)) {
  process.stderr.write(usage)
  process.exitCode = 2
} else {
  try {
    const report = activation ? await readActivationPlan(args[0], args[1], origin) : context ? await (action === 'review-context' ? exportLegacyContextReview : applyLegacyContextSummary)(args[0], origin, args[2], args[3]) :
