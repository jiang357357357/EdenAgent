...
const appName = process.env.MON_AGENT_SERVER_PM2_NAME || "agent-api";
...
const processLogFile = logStartDir ? path.join(logStartDir, "Process", "monagent_server_process.log") : "";
  ...
  apps: [
    {
      name: appName,
      cwd: projectRoot,
      script: path.join(projectRoot, "Script/Process/linux/server/run_server.sh"),
      interpreter: "bash",
      autorestart: true,
      max_restarts: 10,
      restart_delay: 3000,
