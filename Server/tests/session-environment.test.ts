...
test('environment updates commit with queued input, survive reopen, and reach the model without coordinates', async () => {
  ...
  try {
    ...
    assert.throws(() => routes['turn.start']!({ sessionId: session.id, text: 'Hello', environment, idempotencyKey: 'env' }), /environment disk failure/)
    ...
    routes['turn.start']!({ sessionId: session.id, text: 'Hello', environment, idempotencyKey: 'env' })
