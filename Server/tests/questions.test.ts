...
test('the model asks a durable question and continues only after a validated answer commits', async () => {
  ...
  try {
    ...
    assert.throws(() => routes['question.resolve']!({ requestId: question.id, answers: [['Unknown']] }), /listed option/)
    assert.throws(() => routes['question.resolve']!({ requestId: question.id, answers: [['JSON', 'Text']] }), /one answer/)
    ...
    assert.throws(() => routes['question.resolve']!({ requestId: question.id, answers: [['JSON']] }), /answer disk failure/)
    assert.equal(questions.list().length, 1)
    assert.equal(model.requests.length, 1)
    db.connection.exec('DROP TRIGGER reject_answer')
    routes['question.resolve']!({ requestId: question.id, answers: [['JSON']] })
    await sessions.waitForIdle(session.id)
    assert.equal(sessions.faultCount(), 0)
    assert.deepEqual(routes['question.list']!({}), [])
    ...
    assert.throws(() => routes['question.reject']!({ requestId: question.id }), /already resolved/)
