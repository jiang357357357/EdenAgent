...
test('candidate RPC resumes persisted candidates with exact revision and session without another model request', async context => {
  ...
  assert.deepEqual(routes['memory.extraction.candidates']!({ sessionId: session.id }), page)
  const params = { sessionId: session.id, jobId: item.id, revision: item.revision }
  assert.throws(() => routes['memory.extraction.resume']!({ ...params, revision: '0'.repeat(64) }), /changed/)
  const other = f.server.sessions.repository.create('Other session')
  assert.throws(() => routes['memory.extraction.resume']!({ ...params, sessionId: other.id }), /belong/)
  assert.throws(() => routes['memory.extraction.resume']!({ ...params, candidates: [] }))
  assert.deepEqual(routes['memory.extraction.resume']!(params), { jobId: item.id, state: 'accepted' })
  assert.deepEqual(routes['memory.extraction.resume']!(params), { jobId: item.id, state: 'accepted' })
