...
export function loadConfig(env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): ServerConfig {
  ...
  return { origin, migrationReview, ...(selection ? { selection } : {}), host: '127.0.0.1', port, dataRoot, databasePath: path.join(dataRoot, migrationReview ? 'agent.sqlite' : 'eden-agent.db'), token,
