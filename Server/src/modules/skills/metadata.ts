...
const frontmatterSchema = z.object({
  ...
  metadata: z.object({ edenagent: z.object({ display_name: text.optional(), version: z.union([text, z.number()]).optional(),
...
export function skillMetadata(content: string, fallback: string) {
  ...
  return { name, displayName: eden?.display_name || fields['display-name'] || name, description: fields.description ?? '',
