...
const payloadSchema = z.object({ prompt: z.string().optional(), trigger: jsonValue.optional(),
...
export class SelfAwakeJobRecovery {
  ...
  preview(id: string) {
    ...
    return { fingerprint: createHash('sha256').update(JSON.stringify(basis)).digest('hex'), job: basis.job,
  ...
  resubmit(id: string, fingerprint: string, note: string): JobInfo {
    return this.database.transaction(() => {
      ...
      const next = this.jobs.scheduleInTransaction({ kind: 'self_awake', sessionId: basis.job.sessionId, dueAt: Date.now(),
