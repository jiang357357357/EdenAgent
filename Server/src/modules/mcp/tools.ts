...
export function mcpTools(lifecycle: McpLifecycle, database: EdenDatabase, permissions: PermissionService, sessionId: string, turnId: string): RuntimeTool[] {
  const tools: RuntimeTool[] = [{ name: 'list_mcp_servers', revision: 'eden.mcp.v1', description: 'List active MCP plugin runtimes and initialization errors.',
    parameters: { type: 'object', properties: {}, additionalProperties: false },
    async execute(raw) { z.object({}).strict().parse(raw); return toJson(lifecycle.list()) } },
  ...(['catalog', 'call', 'read'] as const).map(kind => ({ name: kind === 'catalog' ? 'list_mcp_capabilities' : kind === 'call' ? 'call_mcp_tool' : 'read_mcp_resource',
