...
const payload = z.object({ pluginId: z.string(), revision: z.string(), hookId: z.string(), skillName: z.string(), eventId: z.string(), event: z.string(), occurredAt: z.number() }).strict()
export class PluginHookService {
  ...
  dispatch(job: JobInfo) {
    ...
    const input = payload.parse(job.payload)
    ...
    if (!hook || !job.sessionId) throw new Error('Plugin hook was disabled, changed or removed before dispatch')
