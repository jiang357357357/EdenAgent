...
const stdio = z.object({ schemaVersion: z.literal(1).default(1), command: z.string().min(1).max(4096).refine(value => !/[\r\n\0]/.test(value)),
...
export function packageRuntimeDescriptors(value: Package, enabled: (id: string, fallback: boolean) => boolean) {
  return value.manifest.components.runtimes.filter(item => enabled(item.id, item.enabledByDefault)).map(component => {
    ...
    if (component.kind === 'mcp_stdio') {
      ...
      return { ...owner, kind: 'mcp_stdio' as const, descriptor,
    ...
    if (component.kind === 'mcp_http') {
      ...
      return { ...owner, kind: 'mcp_http' as const, descriptor,
