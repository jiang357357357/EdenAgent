...
export function verifyPackageFiles(files: Map<string, Buffer>, trustedKey: (id: string) => string, allowUnsigned = false) {
  ...
  const checksums = checksumsBytes ? z.record(z.string(), z.string().regex(/^[a-fA-F0-9]{64}$/)).parse(JSON.parse(checksumsBytes.toString('utf8'))) : null
  const contentNames = [...files.keys()].filter(name => name !== 'checksums.json' && name !== 'signature.json').sort()
  ...
  const summary = { revision: createHash('sha256').update(manifestBytes).update(digest).digest('hex'), manifest: JSON.parse(manifestBytes.toString('utf8')), files: contentNames, totalBytes: contentNames.reduce((sum, name) => sum + files.get(name)!.length, 0) }
  ...
  return { revision: createHash('sha256').update(manifestBytes).update(digest).digest('hex'), keyId: signature.keyId,
    manifest: JSON.parse(manifestBytes.toString('utf8')), files: contentNames, totalBytes: contentNames.reduce((sum, name) => sum + files.get(name)!.length, 0) }
}
