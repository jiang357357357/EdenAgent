...
export async function launchOpenTtd(id: string, dataRoot: string, repository: ConnectorRepository, catalog: ConnectorCatalog,
  ...
  if (current.connectorKey !== 'openttd') throw new Error('Invalid OpenTTD connector')
  ...
  try {
    ...
    const child = process
    const exited = child.exited.finally(() => bridge.close())
    ...
    try {
      ...
      await runtime.initialize({ protocolVersion: 1, connectorInstanceId: id, connectorKey: 'openttd',
        packageVersion: catalog.descriptor('openttd').manifest.version, settings,
