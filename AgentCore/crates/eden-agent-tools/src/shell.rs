...
fn resolve_bash() -> Result<PathBuf, ToolFailure> {
    if let Some(configured) = env::var_os("EDEN_AGENT_SHELL").filter(|value| !value.is_empty()) {
    ...
    candidates.into_iter().find(|path| path.is_file()).ok_or_else(|| {
        fail(
            "bash_unavailable",
            "Bash is unavailable. Install Bash or set EDEN_AGENT_SHELL to its executable path.",
        )
    })
}

fn resolve_powershell() -> Result<PathBuf, ToolFailure> {
    if let Some(configured) = env::var_os("EDEN_AGENT_POWERSHELL").filter(|value| !value.is_empty()) {
        let path = PathBuf::from(&configured);
        if path.is_file() {
            return Ok(path);
        }
    ...
    Err(fail(
        ...
        "PowerShell is unavailable. Install PowerShell or set EDEN_AGENT_POWERSHELL to its executable path.",
    ))
}
