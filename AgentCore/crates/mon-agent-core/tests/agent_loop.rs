...
impl ModelAdapter for ScriptedModel {
    ...
    ) -> Result<mon_agent_core::ModelOutput, ModelError> {
        self.requests.lock().unwrap().push(request);
        self.responses
            .lock()
            .unwrap()
            .pop_front()
            .map(mon_agent_core::ModelOutput::complete)
...
async fn run_and_collect(driver: AgentLoop, control: LoopControl) -> (mon_agent_core::RunResult, Vec<AgentEvent>) {
...
async fn queued_follow_up_opens_another_turn() {
    ...
    let control = LoopControl {
        steering: mon_agent_core::PendingMessageQueue::new(QueueMode::OneAtATime),
        follow_up: mon_agent_core::PendingMessageQueue::new(QueueMode::OneAtATime),
    };
    ...
    assert!(model.requests()[1].messages.iter().any(|message| {
        match message {
            Message::User {
                content: mon_agent_core::UserContent::Blocks(blocks),
                ..
            } => blocks
                .iter()
