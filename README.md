...
数据固定存放在 `Data/realms/mon` 与 `Data/realms/local`。旧 Agent 数据不会读取、迁移或自动复制，数据库永久绑定世界。
...
模型默认只接收当前场景的基础工具。专业能力通过 `list_tools` 发现、`load_tools` 加载；技能通过 `list_skills` 发现、`load_skill` 读取说明和关联工具信息。内置记忆、提醒、工作区和自醒四份基础技能，按场景默认提供名称与概要，正文按需读取。工具接口与技能阅读状态独立，读取技能不加载工具：技能代码工具也可直接通过 `load_tools` 获取，已知用法时无需重读技能。`unload_skill` 只停用技能加载记录，释放工具接口使用 `unload_tools`。

加载状态按会话和角色保存，版本或工作区变化后重新核对。加载不会自动批准执行，通用入口与具名工具共用最终目标权限检查。详见[工具与技能按需加载](文档/技术/Eden%20Agent%20工具与技能按需加载.md)。

## 自写插件

智能体使用 `manage_plugins` 读取开发指南、新建或读取草稿、提交修订、验证、声明测试、安装和激活精确版本。插件开发页提供相同业务入口、源码编辑、差异、运行记录及编辑备份。
...
在插件管理中从该制品目录预览、安装、批准包权限并启用，再配置连接器实例及具体资源授权。旧连接器实例不会自动继承新插件版本的授权。智能体通过 `manage_connector_plugins` 获取 SDK/构建流程并预览安装自己的 TS 插件，不能自行授予权限。
