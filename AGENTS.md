...
3. 数据默认分别使用 `Data/realms/mon` 与 `Data/realms/local`。旧 Agent 数据不读取、不迁移；测试只使用临时目录。
...
4. 2026-09-10 用户最新要求：暂停全部执行沙箱，终端、插件、技能、MCP、连接器和 Git 源获取统一按当前 OS 账户权限本机运行。沙箱机制必须经开发者审阅后才能重新加入；原实现保存在 `Archive/2026-09-10-sandbox-paused`，不参与运行。无环境变量或界面开关可重新开启。审批、版本校验、进程超时与取消继续生效；两个世界没有 OS 文件/网络隔离。子智能体原 sandboxMode 仅保留工具策略含义，不代表 OS 沙箱。
