...
export async function stageLegacySessions(snapshot: string, destination: string, origin: 'mon' | 'local', blobSourceRoot?: string, pluginVersionsRoot?: string) {
  ...
  try {
    ...
    return await runLegacyConversion(database.connection, source, target, origin, blobSourceRoot, pluginVersionsRoot)
