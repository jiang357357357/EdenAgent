...
export function applyLegacyEventPatch(base: unknown, raw: unknown, depth = 0): unknown {
  if (depth > 128 || !raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('Invalid legacy event patch')
  ...
  if (!changes || typeof changes !== 'object' || Array.isArray(changes)) throw new Error('Invalid legacy event patch changes')
  if (tag === 'Object' && base && typeof base === 'object' && !Array.isArray(base) && Array.isArray(extra) && extra.every(key => typeof key === 'string')) {
  ...
  if (tag === 'Array' && Array.isArray(base) && Number.isSafeInteger(extra) && extra >= 0 && extra <= 1000000) {
