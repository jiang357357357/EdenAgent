...
export function DialogInstanceSelect() {
  const dialog = useDialog()
  const toast = useToast()
  const { config, load, setChatActive, removeInstance } = useInstances()
  ...
  async function onSelect(option: { value: string }) {
    ...
    const current = config().chat_active
    await setChatActive(id === current ? null : id)
    toast.show({ message: id === current ? "已取消选择" : "已设为活跃对话模型", variant: "info" })
    dialog.clear()
  }
