...
export function runHostCommand(root: string, command: string, signal: AbortSignal) {
  ...
  return runProcess({ executable: host.shell, args, cwd: root, input: '', timeoutMs: 30000, maxOutputBytes: 1024 * 1024, signal })
}
