...
export async function runSkillCommand(root: string, command: readonly string[], input: unknown, timeoutSeconds: number, signal: AbortSignal, external?: ExternalCommandSandbox) {
  if (!command.length) throw new Error('Skill command is empty')
  if (external) return runProcess({ ...external.program(root, root, command), input: JSON.stringify(input), timeoutMs: timeoutSeconds * 1000, maxOutputBytes: 1024 * 1024, signal })
