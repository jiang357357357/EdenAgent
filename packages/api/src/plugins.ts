...
export const pluginDraftOperationSchema = pluginIdSchema.extend({ expectedDraftRevision: z.string().regex(/^[a-f0-9]{64}$/).optional() })
...
export const pluginDraftSchema = z.object({ expectedDraftRevision: z.string().regex(/^[a-f0-9]{64}$/).nullable().optional(), manifest: jsonValue, source: z.string().max(65536) }).strict()
...
export const pluginManageSchema = z.object({
  action: z.enum(['describe', 'read', 'draft', 'validate', 'test', 'install', 'activate', 'disable', 'list', 'invoke']),
  args: jsonValue,
}).strict()
