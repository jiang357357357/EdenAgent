...
import { pluginDraftOperationSchema, pluginIdSchema, pluginVersionSchema, pluginActivationSchema, pluginDraftSchema, pluginGrantSchema } from './plugins.ts'
...
export const pluginDraftContentSchema = z.object({ manifest: jsonValue, source: z.string(), draftRevision: z.string().regex(/^[a-f0-9]{64}$/).optional() })
...
export const pluginDevelopmentRpcMethods = {
  ...
  'plugin.describe': { params: z.object({}).strict(), result: z.object({ manifestExample: jsonValue, manifestSchema: jsonValue, sourceExample: z.string(),
  ...
  'plugin.validate': { params: pluginDraftOperationSchema, result: pluginVersionSchema.extend({ valid: z.literal(true), draftRevision: z.string().regex(/^[a-f0-9]{64}$/) }) },
  'plugin.test': { params: pluginDraftOperationSchema, result: pluginTestReportSchema },
