# Eden Agent Hearts of Iron IV 观察连接器
...
HOI4 Observer Mod
    -> logs/game.log
    -> eden-agent-connector-hoi4 Worker
...
Server 的依赖树中没有 `eden-agent-hoi4`。官方与第三方连接器使用同一种包和进程
...
EDENAGENT_HOI4|1|SNAPSHOT|date=1939.9.1|country_tag=GER|...
```
...
`Eden Agent Hearts of Iron IV Observer Bridge` 并进入一次战役。

实机验收条件：

1. `logs/game.log` 出现 `EDENAGENT_HOI4|1|HELLO`。
2. 玩家国家出现一条 `SNAPSHOT`，月度推进后出现下一条快照。
3. `error.log` 不包含 `edenagent_hoi4` 相关脚本错误。
4. Eden Agent 连接器状态为已连接，`bridgeSeen=true`。
