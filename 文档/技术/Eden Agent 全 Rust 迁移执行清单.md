# Eden Agent 全 Rust 迁移执行记录

状态：架构代码切换完成（不等同于完整产品功能验收）  
完成日期：2026-08-19  
目标方案：[Eden Agent 全 Rust 服务端长期架构方案](./Eden Agent%20全%20Rust%20服务端长期架构方案.md)

> 本文件记录 Python host/sidecar 到单进程 Rust 架构的切换历史。完整产品能力迁移尚按 [Eden Agent 全 Rust 完整功能迁移计划](./Eden Agent%20全%20Rust%20完整功能迁移计划.md) 和 [Eden Agent 归档行为验收矩阵](./Eden Agent%20归档行为验收矩阵.md) 持续验收；不得用本文件中的“完成”替代逐项运行证据。

## 1. 最终结论

Eden Agent 的产品运行链已经迁移为单一 Rust Server：

```text
frontend/web + Electron
          │
          ▼
eden-agent-server（Axum / Tokio / SQLite）
...
- Electron 只启动和监控 `eden-agent-server`，并通过私有 token 文件完成能力令牌交付。
...
| AgentCore | `eden-agent-domain`、`eden-agent-core`、`eden-agent-tools` | 完成 |
| Server | `eden-agent-server` Rust binary，直接依赖 AgentCore | 完成 |
...
- Windows 当前没有内置的等价命令沙箱，因此 shell fail closed；可通过 `EDEN_AGENT_SANDBOX_EXECUTABLE` 配置经过审计的外部隔离器。
...
归档包括：Python Server 源码与测试、venv/cache、`pyproject.toml`、`uv.lock`、Python 启动脚本、sidecar 发布目录、`eden-agent-protocol`、`eden-agent-runtime`、旧打包脚本和旧迁移说明。
