...
`eden-agent-connectors` 不再按 connector key 硬编码游戏或服务实现。
...
可以要求 Worker 去重。Worker 无权访问 Eden Agent SQLite，也不能指定另一个连接器 ID。
...
1. 引入 `eden-agent-connector-protocol`、`package` 和 `host`。
...
- `eden-agent-connector-protocol`：8 MiB 上限的长度前缀 JSON、请求/响应/通知类型。
- `eden-agent-connector-package`：目录热发现、严格 manifest、平台入口、完整性和坏包隔离。
- `eden-agent-connector-host`：最小环境启动、握手、能力核验、超时、通知和有序退出。
- `eden-agent-connectors`：只通过包 Worker 运行，负责状态同步、事件持久化和通用查询/动作工具。
