...
- 收件地址由用户核对，且允许接收一封标题为“MonAgent 全 Rust 迁移验收”的邮件。
...
cargo test -p mon-agent-host core_tools::tests::real_core_email_delivery_is_confirmed_by_the_transport -- --ignored --exact --nocapture
...
```powershell
$env:MON_TEST_ALLOW_EXTERNAL_SEND = 'I_UNDERSTAND_THIS_SENDS_A_REAL_MESSAGE'
cargo test -p mon-agent-host core_tools::tests::real_core_qq_delivery_requires_a_confirmed_bot_ack -- --ignored --exact --nocapture
Remove-Item Env:MON_TEST_ALLOW_EXTERNAL_SEND
```
...
前置条件：Linux、Node.js 22+、OpenTTD 可执行文件、基础配置、测试存档、环回 Admin Port 密码，以及安装到 OpenTTD `game` 内容目录的 MonAgentBridge GameScript。
