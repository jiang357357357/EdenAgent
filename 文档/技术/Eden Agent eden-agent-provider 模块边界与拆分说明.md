# Eden Agent eden-agent-provider 模块边界与拆分说明

## 定位

`eden-agent-provider` 是 Server 内的模型供应商适配层。它实现 AgentCore 的
...
```text
eden-agent-app / eden-agent-server
              |
              v
...
```powershell
cargo fmt -p eden-agent-provider -- --check
cargo check -p eden-agent-provider
cargo test -p eden-agent-provider
