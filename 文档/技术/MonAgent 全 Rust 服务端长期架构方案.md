...
- 最终产品只有一个 `mon-agent-server` Rust 服务进程，不再由 Python 启动 sidecar，也不再通过 NDJSON/stdio 在 Server 与 AgentCore 之间通信。
...
```text
frontend/web ─┐
              ├─ WebSocket/HTTP ─> mon-agent-server
Electron ─────┘                         │
                                       ├─ mon-agent-core
                                       ├─ mon-agent-domain
                                       └─ mon-agent-tools
...
实施结果：仓库根现在是唯一 Cargo workspace；`Server` 是 `mon-agent-server` Rust binary，并以 path dependency 直接链接 `mon-agent-domain`、`mon-agent-core` 与 `mon-agent-tools`。客户端只使用鉴权 WebSocket JSON-RPC 和 Blob HTTP 端点。原 Python Server、旧协议 crate、runtime sidecar、uv 配置与打包脚本均已退出工作区。旧源码被可恢复地归档到 `D:\Mon\归档\AgentMigrationArchive_20260819`，不参与构建或运行。
...
Web / Electron
      ...
      ▼
mon-agent-server (Rust)
      ├─ mon-agent-app / store / provider / sandbox
...
Python Server
      │ NDJSON over stdio
      ▼
mon-agent-runtime sidecar
      │
      ▼
AgentCore crates
```

迁移前 Python Server 约有 2.85 万行 Python，复杂度主要集中在 `runtime`、`tools`、`skills`、`connectors`、`llm`、`http`、`store` 和 `native_runtime`。`AgentCore` 当时包含 `mon-agent-protocol` 与 `mon-agent-runtime` 两个过渡 crate；它们现已移除。
...
├─ AgentCore
│  └─ crates
│     ├─ mon-agent-domain             # 新增：稳定领域类型和端口
│     ├─ mon-agent-core               # Agent 状态机与事件循环
│     └─ mon-agent-tools              # 工具定义、注册与调度抽象
├─ Server
│  ├─ Cargo.toml                      # mon-agent-server 二进制
│  ├─ src
│  └─ crates
│     ├─ mon-agent-api                # 客户端协议与传输
│     ├─ mon-agent-app                # 应用编排、会话 actor
│     ├─ mon-agent-store              # SQLite、事件日志、投影
│     ├─ mon-agent-provider           # 模型提供商
│     ├─ mon-agent-sandbox            # 权限执行与 OS 隔离
│     └─ mon-agent-connectors         # Mon 与外部连接器
├─ frontend
│  ├─ web
│  └─ desktop
└─ Script
...
resolver = "2"
members = [
  "AgentCore/crates/mon-agent-domain",
  "AgentCore/crates/mon-agent-core",
  "AgentCore/crates/mon-agent-tools",
  "Server",
  "Server/crates/*",
]
```
...
```toml
[dependencies]
mon-agent-domain.workspace = true
mon-agent-core.workspace = true
mon-agent-tools.workspace = true
```
...
- `mon-agent-runtime` 不再作为生产 sidecar。若开发者确实需要 CLI，可另建薄的 `mon-agent-cli`，但它不是 Server 的依赖路径。
- 当前 `mon-agent-protocol` 所表示的 Server-runtime NDJSON 协议应移除。客户端协议由新的 `mon-agent-api` 定义。
...
- 创建 `mon-agent-server`、`mon-agent-api`、`mon-agent-app`。
...
发布检查必须验证安装包不包含 Python 解释器、Python Server 或 `mon-agent-runtime` sidecar。
...
确认这七项之后，第一批实际工作应是“根 workspace + `mon-agent-domain` + Core 进程内 API + Rust Server 空骨架”，而不是先逐文件翻译 Python。这样可以先锁定目标边界，再沿真实端到端链路替换实现。
...
不应逐项目照搬协议或目录。MonAgent 的核心约束是本地嵌入、可靠持久化、严格权限以及与 Mon 业务能力组合，最终设计应以这些约束为准。
