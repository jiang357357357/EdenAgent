...
- `mon-agent-victoria3` 负责日志路径解析、协议解析、日志跟随、最新快照、命令文件生成、Windows 控制台注入和 ACK 等待。
- `mon-agent-connectors` 负责持久连接器生命周期、事件入库和 `query_victoria3` Agent 工具。
- `Server/connectors/victoria3_bridge/mod` 是 Victoria 3 1.13.x 本地 Mod 源码。
- Electron 仍只负责捕获 Victoria 3 窗口；控制台输入由 Rust Server 的 Victoria 3 连接器执行。
...
Agent / execute_connector_action(probe_control)
  ...
  -> Rust 生成 run/monagent_<uuid>.txt
  -> 验证并激活 victoria3.exe 窗口
  -> 打开 Debug 控制台并输入 run monagent_<uuid>
...
日志中可以存在 Jomini 自己的时间和源文件前缀。解析器从 `[MONAGENT]|` 开始读取：

```text
[MONAGENT]|1|HELLO|bridge_version=0.1.0|mode=observe
[MONAGENT]|1|SNAPSHOT|date=1842.03.15|country_id=CHI|country_name=Great Qing|gold_reserves=125000
...
脚本只在 Victoria 3 用户 Mod 目录建立指向仓库源码的目录联接，不修改游戏安装目录。随后在 Paradox Launcher 的 playset 中启用 **MonAgent Victoria 3 Observer Bridge**，并以 Debug Mode 启动游戏。

在 MonAgent 连接器页面添加 `Victoria 3 观察器`，身份可使用 `local`，然后启用连接。默认日志位置为：
...
```text
debug_log = "[MONAGENT]|1|ACK|command_id=<uuid>|status=success|action=probe_control"
```
