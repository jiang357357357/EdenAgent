# MonAgent 前端与 Electron/Core 职责边界

MonAgent 使用本地单机链路，不扫描随机端口，也不在连接失败后切换到其他主机。
...
- Electron 负责桌面窗口生命周期、启动和监管 `mon-agent-server`、选择工作区、屏幕/摄像头捕获，以及向 renderer 交付短期 capability token。Electron 不承载 Agent 业务协议，也不代理普通 RPC。
...
```text
React renderer
  ├─ WebSocket JSON-RPC / Blob ──> mon-agent-server :40092
