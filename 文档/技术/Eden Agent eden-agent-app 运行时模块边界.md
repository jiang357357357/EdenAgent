# Eden Agent eden-agent-app 运行时模块边界

## 定位

`eden-agent-app` 是 Server 内连接持久化服务与 AgentCore 循环的应用编排层。它负责会话串行化、单轮生命周期、上下文恢复与压缩、导演计划执行、流式事件持久化，以及任务完成或失败后的业务收尾。
...
外部代码继续通过 `eden_agent_app::SessionRuntime` 使用运行时，内部模块不构成公共 API。
...
```powershell
cargo fmt --all -- --check
cargo check -p eden-agent-app
cargo test -p eden-agent-app
cargo clippy -p eden-agent-app --all-targets --no-deps -- -D warnings
cargo check -p eden-agent-server
```
