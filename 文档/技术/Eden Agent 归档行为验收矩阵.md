...
- 真实邮件与 QQ 各有默认忽略、二次确认门控的传输验收测试：邮件要求 SMTP 无拒收，QQ 要求 Core 准入通过且 BotCore/NapCat 回执 `delivery.confirmed=true`；当前环境未满足前置条件，未发送测试消息。执行要求见《Eden Agent 真实外部验收手册》。
...
|---:|---|---|---|---|
| 1 | `test_app_state_hydration.py` | 会话级 Core 身份、默认助手和模型绑定，不再使用 Python 全局 hydration cache | `eden-agent-app/prompt.rs`、`eden-agent-provider` 会话绑定测试、`default-assistant-session.test.mjs` | 源码覆盖，待复验 |
...
| 5 | `test_companion_director.py` | Rust Director 生成 scene/execution/beats，并持久化 speaker/orchestration | `eden-agent-app/director.rs`、App 多参与者测试、3 个 Director 前端测试 | 源码覆盖，待复验 |
...
| 10 | `test_core_context.py` | CoreSync durable outbox、canonical context、participants 和 character state | `eden-agent-core-sync` 4 项测试、App prompt 环境测试 | 源码覆盖，待复验 |
...
| 14 | `test_external_files.py` | 旧版任意绝对路径/符号链接读取被取消；改为逐根授权、有界、拒绝逃逸的外部读取 | `eden-agent-workspace` 3 项测试 | 有意替代，安全语义更严格 |
...
| 39 | `test_skill_installer.py` | Rust 完整包 preview/install/update/uninstall、user/project 作用域、文件增删、同级备份、原子替换和内容哈希 | `eden-agent-skills` 11 项测试 | Windows 自动化验证通过 |
...
| 47 | `test_web_tools.py` | Rust structured multi-provider web、缓存、引用、取消、总超时和 SSRF 防护 | `eden-agent-host/web.rs` 10 项测试 | 源码覆盖，待复验 |
